	...
	class Minimum_Requirements {
		...
		public function check_compatibility_on_install() {
			if ( ! $this->is_compatible_version() ) {
				...
				$html = '<div>' . __( 'Activation of ' . $this->name . ' in not possible', 'birds' ) . ':</div><ul>';
				...
				$html .= '</ul>';
				wp_die( $html, __( 'Activation of ' . $this->name . ' in not possible', 'birds' ), array( 'back_link' => true ) ); // XSS ok.
			};
		}
